...
class WaveLoc_NN(tf.keras.Model):
    def __init__(self, config):
        ...
        self.wav_pool_len = tf.constant(np.float32(config['wav_pool_len']))
        self._build_model()
    ...
    def _build_model(self):
        ...
        gt_layer_pool = tf.keras.layers.MaxPool2D(
            [self.wav_pool_len, 1], [self.wav_pool_len, 1])
        ...
        for i in range(self.n_band):
            ...
            band_layer2 = tf.keras.layers.Flatten()
            band_layer3 = tf.keras.layers.Dense(
                units=128, activation=tf.nn.relu)
            band_layer4 = tf.keras.layers.Dense(
                units=32, activation=tf.nn.relu)
            band_layer_all.append(
                [band_layer1, band_layer2, band_layer3, band_layer4])
        #
    ...
    def get_feat_extractors_weights(self):
        ...
        for band_i in range(self.n_band):
            ...
            for layer in self.band_layer_all[band_i]:
                weights_band.append(layer.get_weights())
            weights.append(weights_band)
        return weights
    ...
    def get_feat_extractors_cnn_weights(self):
        ...
        weights = []
        for band_i in range(self.n_band):
            cnn_weights = self.band_layer_all[band_i][0].get_weights()
            weights.append(cnn_weights)
        return weights
    ...
    def set_feat_extractors_cnn_weights(self, weights):
        self.random_run()
        for band_i in range(self.n_band):
            cnn_layer = self.band_layer_all[band_i][0]
            cnn_layer.set_weights(weights[band_i])
    ...
    def get_cnn_kernels(self):
        x_tmp = np.random.rand(5, self.filter_len*2, 2, 1)
        self.call(x_tmp)
        kernel = [self.band_layer_all[band_i][0].get_weights()[0]
                  for band_i in range(self.n_band)]
        return kernel
...
class Model(object):
    ...
    def evaluate(self, file_paths, log_path):
        """
        """
        ...
        for file_i, file_path in enumerate(file_paths):
            x, y = self.file_reader(file_path)
            n_batch = np.int(np.ceil(x.shape[0]/batch_size))
            y_est = []
            for batch_i in range(n_batch):
                batch_slice = slice(batch_i*batch_size,
                                    (batch_i+1)*batch_size)
                y_est_batch = self.nn(x[batch_slice])
                y_est.append(y_est_batch)
            add_log(logger, file_path, tf.concat(y_est, axis=0))
            pb.update()
        logger.close()
